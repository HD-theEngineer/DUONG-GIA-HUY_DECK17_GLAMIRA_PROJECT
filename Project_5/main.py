        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            ...
            while future_to_id:
                ...
                try:
                    ...
                    for future in done_futures:
                        ...
                        if "success" in product_data:
                            ...
                            if retry_count < MAX_RETRIES:
                                ...
                                print(f"Retrying this data: {data_to_retry}")
                    ...
                    print(
                        "Timeout for this url, waiting for other futures to complete..."
                    )
                    # if stop_all_workers.is_set():
                    #     break
                    continue
                    ...
                    print(f"Error processing package: {faulty_package}")
                    continue
    ...
    try:
        ...
        if response.status_code in [200, 201]:
            ...
            if react_data:
                ...
                product_data["value"] = "nothing found"
            return {
            ...
            return {"failed": url, "error": response.status_code}
...
def main():
    ...
    headers_template = {
        ...
        "DNT": "1",
        "Referer": "https://www.ring-paare.de/",
    }

    file_path = ["product_dict\\batch_4_div_2.1.json"]
    test_path = ["product_dict\\test.json"]
    test_err_path = ["product_dict\\test_error.json"]
    ...
    for id_url in streaming_json(test_err_path):
        start_time = time.perf_counter()

        result, faulty_package = product_scraping(id_url, headers_template, user_agents)
        print(f"Crawled result from url:\n{result}")
        print(f"These are the skippe/failed urls:\n{faulty_package}")
        stop_all_workers.clear()

        final_result.append(result)
        final_faulty.extend(faulty_package)
    ...
    if final_faulty:
        ...
        save_to_json(
            final_faulty, output_dir=faulty_output_dir, save_name="faulty_w_err"
        )
    if final_result:
        print(f"Data crawled success: {len(final_result)}")
        save_to_json(
            final_result, output_dir=result_output_dir, save_name="test_result_w_err"
        )
